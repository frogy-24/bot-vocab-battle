...
func (s *Server) Routes() http.Handler {
	...
	mux.HandleFunc("/mailtm", serverlog.WithLogging(s.mailTMEmails))
	mux.HandleFunc("/mailtm/messages", serverlog.WithLogging(s.mailTMMessages))
	mux.HandleFunc("/get-token", serverlog.WithLogging(s.firebaseSignInWithEmailLink))
	mux.HandleFunc("/verify-token", serverlog.WithLogging(s.firebaseVerifyToken))
	mux.HandleFunc("/parroto/register", serverlog.WithLogging(s.firebaseSendOobCode))
